...
<div class="portlet-body panel-content-area">
  <table class="table table-striped table-bordered table-hover" id="ajax-data-table" action-delete="{{route('deleteJobTitle')}}">
  <thead>
  <tr>
  ...
  <tr class="odd gradeX" id="ajax-table-item-{{$job->id}}">
    <td>
      <div class="checker">
        <span>
          <input type="checkbox" class="checkboxes" value="{{ $job->id }}"/>
        </span>
      </div>
    </td>
    <td>{{$job->name}}</td>
    <td>
      ...
      <a item-id="{{$job->id}}" class="btn btn-default btn-xs black remove-item"><i class="fa fa-trash-o"></i> {{trans('all.delete')}}</a>
    </td>
  </tr>
  @endforeach
  </tbody>
...
function saveRow(oTable, nRow, new_mode) {
    ...
    {
      type_method = 'POST';
      route = '{{route('newJobTitlePost')}}';
      value_row = jqInputs[0].value
      data_xhr = 'job_name='+value_row;

    }else
    {
      type_method = 'PUT';
      route = '{{route('putJobTitle')}}';
      value_row = jqInputs[1].value;
      data_xhr = 'job_title_id='+jqInputs[1].getAttribute('item-id')+'&job_name='+value_row;
    }
