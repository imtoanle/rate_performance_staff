...
<div class="portlet-title">
  <div class="caption">
    <i class="fa fa-list"></i>{{trans('all.criteria-list')}}
  </div>
  <div class="actions">
...
<div class="portlet-body panel-content-area">
  <table class="table table-striped table-bordered table-hover" id="ajax-data-table" action-delete="{{route('deleteCriteria')}}">
  <thead>
  <tr>
  ...
  <tr class="odd gradeX" id="ajax-table-item-{{$criteria->id}}">
    <td>
      <div class="checker">
        <span>
          <input type="checkbox" class="checkboxes" value="{{ $criteria->id }}"/>
        </span>
      </div>
    </td>
    <td>{{$criteria->name}}</td>
    <td>
      ...
      <a item-id="{{$criteria->id}}" class="btn btn-default btn-xs black remove-item"><i class="fa fa-trash-o"></i> {{trans('all.delete')}}</a>
    </td>
  </tr>
  @endforeach
  </tbody>
...
function saveRow(oTable, nRow, new_mode) {
    ...
    {
      type_method = 'POST';
      route = '{{route('newCriteriaPost')}}';
      value_row = jqInputs[0].value
      data_xhr = 'criteria_name='+value_row;

    }else
    {
      type_method = 'PUT';
      route = '{{route('putCriteria')}}';
      value_row = jqInputs[1].value;
      data_xhr = 'criteria_id='+jqInputs[1].getAttribute('item-id')+'&criteria_name='+value_row;
    }
