...
<div class="portlet-title">
  <div class="caption">
    <i class="fa fa-list"></i>{{trans('all.rating-type-list')}}
  </div>
  <div class="actions">
...
<div class="portlet-body panel-content-area">
  <table class="table table-striped table-bordered table-hover" id="ajax-data-table" action-delete="{{route('deleteRatingType')}}">
  <thead>
  <tr>
  ...
  <tr class="odd gradeX" id="ajax-table-item-{{$rating_type->id}}">
    <td>
      <div class="checker">
        <span>
          <input type="checkbox" class="checkboxes" value="{{ $rating_type->id }}"/>
        </span>
      </div>
    </td>
    <td>{{$rating_type->name}}</td>
    <td>{{$rating_type->value}}</td>
    <td>
      <a class="edit btn btn-default btn-xs purple"><i class="fa fa-edit"></i> {{trans('all.edit')}}</a>
      <a item-id="{{$rating_type->id}}" class="btn btn-default btn-xs black remove-item"><i class="fa fa-trash-o"></i> {{trans('all.delete')}}</a>
    </td>
  </tr>
  @endforeach
  </tbody>
...
function editRow(oTable, nRow, new_mode) {
    ...
    jqTds[1].innerHTML = '<input type="text" class="form-control input-large" item-id="'+itemId.val()+'" value="' + aData[1] + '">';
    jqTds[2].innerHTML = '<input type="text" class="form-control input-large" value="' + aData[2] + '">';
    jqTds[3].innerHTML = '<a class="edit btn btn-default btn-xs purple" edit-mode="save" '+new_mode_string+'><i class="fa fa-edit"></i> {{trans('all.save')}}</a><a class="cancel btn btn-default btn-xs black" '+new_mode_string+'><i class="fa fa-trash-o"></i> {{trans('all.cancel')}}</a>';
}
...
function saveRow(oTable, nRow, new_mode) {
    ...
    {
      type_method = 'POST';
      route = '{{route('newRatingTypePost')}}';
      row_name = jqInputs[0].value;
      row_value = jqInputs[1].value
      data_xhr = 'rating_type_name='+row_name+'&rating_type_value='+row_value;

    }else
    {
      type_method = 'PUT';
      route = '{{route('putRatingType')}}';
      row_name = jqInputs[1].value;
      row_value = jqInputs[2].value
      data_xhr = 'rating_type_id='+jqInputs[1].getAttribute('item-id')+'&rating_type_name='+row_name+'&rating_type_value='+row_value;
    }
    ajax_call_custom(type_method, route, data_xhr, function(result){
      ...
      <a item-id="'+result.itemId+'" class="btn btn-default btn-xs black remove-item"><i class="fa fa-trash-o"></i> {{trans('all.delete')}}</a>';
        ...
        oTable.fnUpdate(row_name, nRow, 1, false);
        oTable.fnUpdate(row_value, nRow, 2, false);
        oTable.fnUpdate(actions, nRow, 3, false);
      }
    });
    oTable.fnDraw();
}
