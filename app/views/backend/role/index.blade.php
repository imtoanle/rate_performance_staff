...
<div class="portlet-title">
  <div class="caption">
    <i class="fa fa-list"></i>{{trans('all.role-list')}}
  </div>
  <div class="actions">
...
<div class="portlet-body panel-content-area">
  <table class="table table-striped table-bordered table-hover" id="ajax-data-table" action-delete="{{route('deleteRole')}}">
  <thead>
  <tr>
  ...
  <tr class="odd gradeX" id="ajax-table-item-{{$role->id}}">
    <td>
      <div class="checker">
        <span>
          <input type="checkbox" class="checkboxes" value="{{ $role->id }}"/>
        </span>
      </div>
    </td>
    <td>{{$role->name}}</td>
    <td>
      ...
      <a item-id="{{$role->id}}" class="btn btn-default btn-xs black remove-item"><i class="fa fa-trash-o"></i> {{trans('all.delete')}}</a>
    </td>
  </tr>
  @endforeach
  </tbody>
...
function saveRow(oTable, nRow, new_mode) {
    ...
    {
      type_method = 'POST';
      route = '{{route('newRolePost')}}';
      value_row = jqInputs[0].value
      data_xhr = 'role_name='+value_row;
    ...
    }else
    {
      ...
      data_xhr = 'role_id='+jqInputs[1].getAttribute('item-id')+'&role_name='+value_row;
    }
